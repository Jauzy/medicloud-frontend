...
<div id="sidebar-menu">
    ...
    <ul id="side-menu" class="metismenu list-unstyled">
        <template v-for="item in menuItems">
            <li class="menu-title text-dark" v-if="item.isTitle" :key="item.id">
                {{ $t(item.label) }}
            </li>
            <li v-if="!item.isTitle && !item.isLayout && item.role.includes(role)" :key="item.id">
                ...
                <router-link :to="item.link" v-if="!hasItems(item)" :class="` ${item.link == $route.path ? 'sidebar-active' : ''}`" :style="`${isCondensed ? 'margin:unset;' : ''}`">
                    <i :class="`${item.link == $route.path ? 'text-white' : 'text-dark'} ${item.icon}`" v-if="item.icon"></i>
                    <span :class="`${item.link == $route.path ? 'text-white' : 'text-dark'}`">{{ $t(item.label) }}</span>
                    <span :class="`text-dark badge badge-pill badge-${item.badge.variant} float-right`" v-if="item.badge">{{ $t(item.badge.text) }}</span>
                </router-link>
